...
// Validate input
if (!isset($_POST['id']) || !is_numeric($_POST['id']) || empty(trim($_POST['product_name'])) || empty(trim($_POST['product_code']))) {
...
$product_code = trim($_POST['product_code']); // Trim input to remove extra spaces
...
$update_sql = "UPDATE products_name SET product_name = ?, product_code = ? WHERE id = ?";
...
$update_stmt->bind_param("ssi", $product_name, $product_code, $product_id);

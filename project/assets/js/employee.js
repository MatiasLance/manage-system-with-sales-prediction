...
jQuery(function($){
    ...
    $('#search-employee').on('keyup', function() {
        ...
        debounceTimer = setTimeout(() => {
            ...
            fetchEmployees(1, searchQuery); // Fetch data after 5 seconds
    ...
    $.ajax({
        ...
        success: function(response){
            if(response){
                ...
                $('#editedWorkingDepartmentInput').prepend(`<option value="${response.working_department}" selected disabled>${response.working_department}</option>`);
                $('#editedPhoneNumberInput').val(response.phone_number);
                $('#editedDateOfHireInput').val(response.date_of_hire);
                $('#editedJobInput').prepend(`<option value="${response.job}" selected disabled>${response.job}</option>`);
                $('#editedEducationalLevelInput').prepend(`<option value="${response.educational_level}" selected>${response.educational_level}</option>`);
                $('#editedGenderInput').prepend(`<option value="${response.gender}" selected disabled>${response.gender}</option>`);

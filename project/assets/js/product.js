...
function fetchData(page, searchQuery) {
    jQuery.ajax({
        ...
        dataType: 'json',
        success: function(response) {
            jQuery('#data-container, #grains-and-cereals-container, #pos-products-container').empty();
            for (let i = 0; i < response.data.length; i++){
                ...
                if(response.data[i].product_category === 'dairy') {
                    jQuery('#data-container').append(`<tr>
                        ...
                        <td><span class="badge text-bg-success text-capitalize">${response.data[i].status}</span></td>

jQuery(function($){
    ...
    $(document).on('click', '#editProductName', function(){
        updateProductName(productNameId, newProductName,)
    })
    ...
    // Delete Product Name
    $(document).on('click', '#deleteProductName', function(){
...
function updateProduct(
    ...
    const payload = {
        ...
        product_name: retrieveSelectedProductNameInput.val(),
...
function updateProductName(productNameId, newProductName){
    const payload = {
    ...
    $.ajax({
        ...
        success: function(response){
            if(response){
                ...
                retrieveSelectedProductNameInput.find(`option[value="${response.product_name}"]`).remove();
                retrieveSelectedProductNameInput.append(`<option value="${response.product_name}" selected>${response.product_name}</option>`);
                ...
                if (retrieveUnitOfPriceInput.find(`option[value="${response.unit_of_price}"]`).length === 0) {
                    retrieveUnitOfPriceInput.append(`<option value="${response.unit_of_price}" selected>${response.unit_of_price}</option>`);
                }
                retrieveProductStatus.val(response.status)
                if (retrieveProductCategory.find(`option[value="${response.category}"]`).length === 0) {
                    retrieveProductCategory.append(`<option value="${response.category}" selected>${response.category}</option>`);
                }

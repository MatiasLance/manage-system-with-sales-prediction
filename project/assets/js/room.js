let roomDebounceTimer;
let currentSearchRoom = '';

jQuery(function($){
    $('#saveAddedRoomNumber').on('submit', function(e){
        e.preventDefault();
        const data = $(this).serializeArray();
        saveAddedRoomNumber(data)
    })

    $('#searchAvailableRoom').on('keyup', function() {
        clearTimeout(bookingDebounceTimer);
        let searchQuery = $(this).val();

        roomDebounceTimer = setTimeout(() => {
            currentSearchRoom = searchQuery;
            roomList(1, searchQuery); 
        }, 500);
    });

    $(document).on('click', '#retrieveRoom', function(){
        const roomID = $(this).data('id');
        $('#retrieveRoomId').val(roomID);
        retrieveRoom(roomID);
    });

    $('#saveEditedRoomDetail').on('submit', function(e){
        e.preventDefault();
        const payload = $(this).serializeArray();
        updateRoomDetails(payload);
    });

    $(document).on('click', '#confirmRoomDeletion', function(){
        const roomID = $(this).data('id');
        retrieveRoom(roomID);
    });
    ...
    $('#deleteRoom').on('click', function(){
        const payload = {
            id: $('#deleteRoomNumberId').val(),
            password: $('#roomPasswordInput').val(),
        }
...
function retrieveRoom(id){
    jQuery.ajax({
        type: 'GET',
        url: './controller/room/RetrieveRoomController.php',
        data: { id: id },
        dataType: 'json',
...
function saveAddedRoomNumber(data){
    jQuery.ajax({
        type: "POST",
        url: "./controller/room/AddRoomController.php",
        data: data,
        dataType: "json",
        success: function(response) {
            if (response.success) {
...
function roomList(page, searchQuery){
    $.ajax({
        url: './controller/room/ListRoomController.php',
        type: 'GET',
        data: { page: page, search: searchQuery },
        dataType: 'json',
        success: function(response) {
            $('#room-container, #selectedRoom, #retrieveBookingSelectRoomID').empty();
            for (let i = 0; i < response.data.length; i++){
                let roomId = response.data[i].id;
            ...
            $('#room-pagination-links').empty();

            // Previous Button
            $('#room-pagination-links').append(`
                <li class="page-item ${page === 1 ? 'disabled' : ''}">
                    <a class="page-link room-page-link" href="#" data-page="${page - 1}" aria-label="Previous">
            ...
            for (let i = 1; i <= response.total_pages; i++) {
                $('#room-pagination-links').append(`
                    <li class="page-item ${i === page ? 'active' : ''}">
                        <a class="page-link room-page-link" href="#" data-page="${i}">${i}</a>
                    </li>
                `);
            }
            ...
            $('#room-pagination-links').append(`
                <li class="page-item ${page === response.total_pages ? 'disabled' : ''}">
                    <a class="page-link room-page-link" href="#" data-page="${page + 1}" aria-label="Next">
...
function updateRoomDetails(payload){
    jQuery.ajax({
        type: "POST",
        url: "./controller/room/UpdateRoomController.php",
        data: payload,
        dataType: "json",
        success: function (response) {
            if (response.success) {
...
function deleteRoomNumberDetail(payload){
    $.ajax({
        type: 'POST',
        url: './controller/room/DeleteRoomController.php',
        data: payload,
        dataType: 'json',
        success: function(response){
            if(response.success){
